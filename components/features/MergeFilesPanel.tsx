...
export default function MergeFilesPanel({
  files = [],
  onAddFiles,
}: MergeProps) {
        ...
        <div className="space-y-2">
          ...
          {files.length < 2 && (
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="w-full py-3 px-4 bg-primary/5 hover:bg-primary/10 border border-dashed border-primary/30 rounded-xl text-xs font-bold  tracking-wider text-primary flex items-center justify-center gap-2 transition-colors"
            >
      ...
      {isMerged && (
        <div className="bg-primary/5 border border-primary/20 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 relative overflow-hidden">
          ...
          <div className="pt-4 flex justify-center relative z-10">
            <button
              type="button"
              className="flex items-center justify-center gap-2 px-8 py-3 bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-bold  tracking-widest rounded-xl transition-all duration-300 shadow-md shadow-primary/20 active:scale-95"
            >
