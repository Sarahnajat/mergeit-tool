...
function parseAss(source: string): AssDocument {
  ...
  if (startIndex < 0 || endIndex < 0 || textIndex < 0) {
    throw new Error(
      "ASS Events format must contain Start, End, and Text.",
    );
  }
...
function buildSrt(captions: Caption[]): string {
  return (
    captions
      .map(
        (caption, index) =>
          `${index + 1}\r\n${msToSrtTime(caption.start)} --> ${msToSrtTime(
            caption.end,
          )}\r\n${caption.text}`,
      )
      .join("\r\n\r\n") + "\r\n"
...
function makeGroups(
  ...
  if (!Number.isFinite(options.value) || options.value <= 0) {
    throw new Error("Split value must be greater than zero.");
  }
  ...
  if (options.method === "lines") {
    ...
    for (let index = 0; index < captions.length; index += size) {
      groups.push(captions.slice(index, index + size));
    }
  ...
  if (options.method === "files") {
    const count = Math.min(
      captions.length,
      Math.max(1, Math.floor(options.value)),
    );
    const groups: Caption[][] = [];
